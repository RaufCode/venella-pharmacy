                ...
                <div class="flex items-center justify-between h-16">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Track your medication orders</p>
                        </div>
                    </div>
                    <button
                        @click="goBack"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div v-else-if="error" class="max-w-md mx-auto">
                <div class="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
                    ...
                    <h3 class="text-lg font-semibold text-red-800 mb-2">Unable to load orders</h3>
            ...
            <!-- Empty State -->
            <div v-else-if="customerOrders.length === 0" class="text-center py-16">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-2xl font-bold text-gray-800">{{ customerOrders.filter(o => o.status?.toLowerCase() === 'pending').length }}</p>
                    ...
                    <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-2xl font-bold text-gray-800">{{ customerOrders.filter(o => o.status?.toLowerCase() === 'processing').length }}</p>
                    ...
                    <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-2xl font-bold text-gray-800">{{ customerOrders.filter(o => o.status?.toLowerCase() === 'delivered').length }}</p>
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    <h3 class="font-semibold text-gray-800 line-clamp-1">
                                        {{ order.order_items?.[0]?.product?.name || "Order Items" }}
                                    </h3>
                                    <p v-if="order.order_items?.length > 1" class="text-sm text-gray-500 mt-1">
                                        +{{ order.order_items.length - 1 }} more item{{ order.order_items.length > 2 ? 's' : '' }}
                                    </p>
                                </div>
                                <div class="flex-shrink-0">
                                    <span :class="getStatusClasses(order.status)" class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium">
                                        {{ order.status }}
                                    </span>
                                </div>
                            </div>
                        ...
                        <div class="p-4">
                            <div class="space-y-3">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="font-bold text-lg text-green-600">₵{{ order.total_amount }}</span>
                                </div>
                                
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600">Order Date</span>
                                    <span class="text-sm font-medium text-gray-800">{{ formatDate(order.created_at) }}</span>
                                </div>

                                <div v-if="order.order_items?.length" class="pt-2 border-t border-gray-100">
                                    <p class="text-xs text-gray-500 mb-2">Items ({{ order.order_items.length }})</p>
                                    <div class="space-y-1">
                                        <div v-for="item in order.order_items.slice(0, 2)" :key="item.id" class="text-xs text-gray-600">
